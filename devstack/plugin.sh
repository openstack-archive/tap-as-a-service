...
function configure_taas_openvswitch_agent {
    ...
    iniset $conf taas driver neutron_taas.services.taas.drivers.linux.openvswitch_taas.OvsTaasDriver
...
function start_taas_openvswitch_agent {
    run_process taas-openvswitch-agent "python $TAAS_OVS_AGENT_BINARY --config-file $NEUTRON_CONF --config-file $TAAS_OVS_AGENT_CONF_FILE"
}

...
class TestSriovNicTaas(base.TaasTestCase):
    ...
    def test_create_tap_service(self, mock_sriov_utils):
        ...
        root_helper = dict()
        mock_sriov_utils.get_sriov_port_params.\
            return_value = FAKE_PORT_PARAMS
        sriov_nic_taas.SriovNicTaasDriver(root_helper).\
    ...
    def test_create_tap_service_no_pf_device_and_vf_index(
        ...
        mock_sriov_utils.get_sriov_port_params.\
            return_value = FAKE_PORT_PARAMS
        self.assertIsNone(sriov_nic_taas.SriovNicTaasDriver(root_helper).
                          create_tap_service(tap_service))
        mock_sriov_utils.get_sriov_port_params.\
            assert_called_once_with(tap_service['port'])
    ...
    def test_create_tap_flow(self, mock_sriov_utils):
        ...
        mock_sriov_utils.get_sriov_port_params.side_effect = [
            src_port_params, ts_port_params]
        mock_sriov_utils.get_list_from_ranges_str.\
            side_effect = [[4, 6, 10, 11], [4, 5, 6, 7, 9, 11, 12]]
        mock_sriov_utils.get_ranges_str_from_list.return_value = '4-7, 11'
        sriov_nic_taas.SriovNicTaasDriver(root_helper).\
            create_tap_flow(tap_flow)
        mock_sriov_utils.execute_sysfs_command.assert_called_once_with(
            'add', ts_port_params, src_port_params, '4-7, 11',
            False, 'IN')
    ...
    def test_create_tap_flow_no_vlan_filter_on_source_and_probe(
        ...
        ts_port_params['vlan_mirror'] = None
    ...
    def test_create_tap_flow_no_source_pci_slot(
        ...
        mock_sriov_utils.get_sriov_port_params.side_effect = [
            src_port_params, ts_port_params]
        self.assertRaises(
            taas.PciSlotNotFound,
            sriov_nic_taas.SriovNicTaasDriver(root_helper).create_tap_flow,
            tap_flow)
    ...
    def test_delete_tap_flow(self, mock_sriov_utils):
        ...
        mock_sriov_utils.get_ranges_str_from_list.return_value = '4-5, 9'
        self.assertIsNone(sriov_nic_taas.SriovNicTaasDriver(root_helper).
                          delete_tap_flow(tap_flow))
        self.assertEqual(2, mock_sriov_utils.execute_sysfs_command.
                         call_count)
    ...
    def test_delete_tap_flow_no_source_pci_slot(
        ...
        self.assertRaises(
            taas.PciSlotNotFound,
            sriov_nic_taas.SriovNicTaasDriver(root_helper).delete_tap_flow,
            tap_flow)

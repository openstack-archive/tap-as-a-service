...
from neutron_taas.osc.taas import tap_flow
...
class TestListTapFlowGroup(fakes.TestNeutronClientOSCV2):
    _tapflow = fakes.FakeTapFlowGroup.create_tap_flows(count=1)
    columns = ('ID', 'Name', 'Tap Flow')
    data = []
    ...
    def setUp(self):
        super(TestListTapFlowGroup, self).setUp()

        self.neutronclient.list_ext = mock.Mock(
            return_value=self._tap_flow1
        )
        # Get the command object to test
        self.cmd = tap_flow.ListTapFlowGroup(self.app, self.namespace)
...
class TestCreateTapFlowGroup(fakes.TestNeutronClientOSCV2):
    # The new tap_flow created
    _tap_flow = fakes.FakeTapFlowGroup.create_tap_flow()
    ...
    def setUp(self):
        super(TestCreateTapFlowGroup, self).setUp()
        ...
        # Get the command object to test
        self.cmd = tap_flow.CreateTapFlowGroup(self.app,
                                               self.namespace)
...
class TestDeleteTapFlowGroup(fakes.TestNeutronClientOSCV2):

    def setUp(self):
        super(TestDeleteTapFlowGroup, self).setUp()
        _tap_flow = fakes.FakeTapFlowGroup.create_tap_flows()
        ...
        self.cmd = tap_flow.DeleteTapFlowGroup(self.app,
                                               self.namespace)
...
class TestShowTapFlowGroup(fakes.TestNeutronClientOSCV2):

    _tf = fakes.FakeTapFlowGroup.create_tap_flow()
    data = (
        _tf['description'],
        _tf['group_id'],
        _tf['id'],
    ...
    def setUp(self):
        super(TestShowTapFlowGroup, self).setUp()
        ...
        # Get the command object to test
        self.cmd = tap_flow.ShowTapFlowGroup(self.app, self.namespace)
...
class TestUpdateTapFlowGroup(fakes.TestNeutronClientOSCV2):
    _tap_flow = fakes.FakeTapFlowGroup.create_tap_flow()
    ...
    def setUp(self):
        super(TestUpdateTapFlowGroup, self).setUp()
        common.update_taas_resource = mock.Mock(return_value=None)
        common.find_taas_resource = mock.Mock(
            return_value=self._tap_flow_id)

        self.cmd = tap_flow.UpdateTapFlowGroup(self.app,
                                               self.namespace)

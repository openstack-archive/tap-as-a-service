...
class sriov_nic_utils(object):
    ...
    #
    def execute_sysfs_command(self, command, pf, tap_service_vf_index, source_vf,
                             common_vlans_ranges_str, vf_to_vf_all_vlans, direction):
        """
        ...
        # Remove traffic mirroring from VF0 to VF31 on interface p2p1, ex.
        # echo rem 3 > /sys/class/net/p2p1/device/sriov/0/ingress_mirror
        # echo rem 3 > /sys/class/net/p2p1/device/sriov/0/egress_mirror

        # Add VLANs 2,4,6,18-22 to Mirror traffic to VF3 (port p2p1), ex.
        # echo add 2,4,6,18-22 > /sys/class/net/p2p1/device/sriov/3/vlan_mirror

        # Remove VLANs 2,4,6,18-22 to Mirror traffic to VF3 (port p2p1), ex.
        # echo rem 2,4,6,18-22 > /sys/class/net/p2p1/device/sriov/3/vlan_mirror

        # Remove all VLANs from mirroring at VF3, ex.
        # echo rem 0-4095 > /sys/class/net/p1p1/device/sriov/3/vlan_mirror
        []
        """
        if vf_to_vf_all_vlans == True:
            if direction == 'OUT' or direction == 'BOTH':
                sysfs_kobject_full_path = \
                    '/sys/class/net/' + pf + '/device/sriov/' + source_vf + '/egress_mirror/'
                commit_cmd = ['echo', command, tap_service_vf_index, '>', sysfs_kobject_full_path]

                if not os.path.exists(sysfs_kobject_full_path):
                    raise n_exc.Invalid("Invalid PF (%s) or Source VF (%s) combination" %
                                        (pf, source_vf))
            ...
            if direction == 'IN' or direction == 'BOTH':
                sysfs_kobject_full_path = \
                    '/sys/class/net/' + pf + '/device/sriov/' + source_vf + '/ingress_mirror/'
                commit_cmd = ['echo', command, tap_service_vf_index, '>', sysfs_kobject_full_path]
            ...
            if direction != 'BOTH':
                LOG.warning("SRIOV NIC Switch driver only supports direction=BOTH "
                            "for specific VLANs' mirroring")

            sysfs_kobject_full_path = \
                '/sys/class/net/' + pf + '/device/sriov/' + tap_service_vf_index + '/'
            commit_cmd = ['echo', command, common_vlans_ranges_str, '>', sysfs_kobject_full_path]

            if not os.path.exists(sysfs_kobject_full_path):
                raise n_exc.Invalid("Invalid PF (%s) or Tap-service VF (%s) combination" %
                                    (pf, tap_service_vf_index))
    ...
    def get_ifname_by_pci_address(self, pci_addr, pf_interface=False):
        ...
        try:
            ...
            raise taas.TapServiceNotFound(id=pci_addr)
    ...
    def get_list_from_ranges_str(self, ranges_str):
        """
        Convert the range in string format to ranges list and yield the merged ranges in order.
        The argument must be a string having comma separated vlan and vlan-ranges.

        # get_list_from_ranges_str("4,6,10-13,25-27,100-103")
        # [4, 6, 10, 11, 12, 13, 25, 26, 27, 100, 101, 102, 103]
        []
        """
        return sum(((list(range(*[int(range_start) + range_index for range_index, range_start in enumerate(range_item.split('-'))]))
            if '-' in range_item else [int(range_item)]) for range_item in ranges_str.split(',')), [])

    def get_ranges_str_from_list(self, ranges):
        """
        Convert the ranges list to string format and yield the merged ranges in order in string format.
        The argument must be an iterable of pairs (start, stop).

        # get_ranges_str_from_list([4, 6, 10, 11, 12, 13, 25, 26, 27, 100, 101, 102, 103])
        # "4,6,10-13,25-27,100-103"
        []
        """
        ranges_str = []
        ...
        return ",".join([str(range_item[0]) if len(range_item) == 1
                         else str(range_item[0]) + "-" + str(range_item[-1]) for range_item in ranges_str])

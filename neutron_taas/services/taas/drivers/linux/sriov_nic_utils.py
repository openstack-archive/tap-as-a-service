...
class SriovNicUtils(object):
    ...
    def get_list_from_ranges_str(self, ranges_str):
        """
        Convert the range in string format to ranges list and yield the
        merged ranges in order. The argument must be a string having comma
        separated vlan and vlan-ranges.
    ...
    def get_ranges_str_from_list(self, ranges):
        """
        Convert the ranges list to string format and yield the merged ranges
        in order in string format.

...
class TaasOvsAgentRpcCallback(api.TaasAgentRpcCallbackMixin):
    ...
    def _taas_rpc_setup(self):
        ...
        self.taas_plugin_rpc = TaasOvsPluginApi(taas_topics.TAAS_PLUGIN,
                                                self.conf.host)
        self.topic = taas_topics.TAAS_AGENT
        self.endpoints = [self]
        consumers = [
            [taas_topics.TAPSERVICE, topics.CREATE],
            [taas_topics.TAPSERVICE, topics.DELETE],
            [taas_topics.TAPFLOW, topics.CREATE],
            [taas_topics.TAPFLOW, topics.DELETE],
        ]

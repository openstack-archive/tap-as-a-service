...
class TaasRpcDriver(service_drivers.TaasDriver):
    ...
    def create_tap_service_postcommit(self, context, tap_service):
        ...
        tap_id_association = self.service_plugin.get_tap_id_association(
            context, tap_service_id=tap_service['id'])
        ...
        rpc_msg = {'tap_service': tap_service,
    ...
    def delete_tap_service(self, context, tap_service_id):
        ...
        """
        tap_id_association = self.service_plugin.get_tap_id_association(
            context, tap_service_id=tap_service_id)
        taas_vlan_id = (tap_id_association['taas_id'] +
                        cfg.CONF.taas.vlan_range_start)
        ts = self.service_plugin.get_tap_service(context, tap_service_id)
    ...
    def create_tap_flow_postcommit(self, context, tap_flow):
        ...
        taas_id = self._get_taas_id(context, tap_flow)
    ...
    def delete_tap_flow(self, context, tap_flow_id):
        ...
        tf = self.service_plugin.get_tap_flow(context, tap_flow_id)
        taas_id = self._get_taas_id(context, tf)
        # Extract the host where the source port is located
        port = self.service_plugin._get_port_details(context,
                                                     tf['source_port'])
        host = port['binding:host_id']

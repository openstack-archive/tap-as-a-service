...
class CreateTapFlow(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            'port',
            help=_('Source port to which the Tap Flow is connected (name or ID)'))
        parser.add_argument(
            'tap-service',
            help=_('Tap Service with which the tapflow is associated (name or ID)'))
        parser.add_argument(
            '--direction',
            required=True,
            choices=['IN', 'OUT', 'BOTH'],
            type=common.convert_to_uppercase,
            help=_('Direction of the packet flow which needs to be mirrored by the '
                   'tapflow'))
        return parser
